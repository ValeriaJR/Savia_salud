...
export default function Home() {
  ...
  const subirArchivo = () => {
    ...
    try {
      const response = axios.post("http://localhost:8000/api/cargar-archivos/", data).then(() => setMostrar(true))
       return response
    ...
    <>
      ...
      <div className="w-full h-screen back flex flex-col justify-start items-center">
        ...
        <form  onSubmit={mano}>
          ...
          {mostrar && <><a target="_blank" rel="noopener noreferrer" className='button px-4 py-1'>Dados de alta</a>
            <a target="_blank" rel="noopener noreferrer" className='button px-4 py-1'>En hospitalización</a>
            <a target="_blank" rel="noopener noreferrer" className='button px-4 py-1'>Nuevos</a></>
          }
          </div>
        </form>
      </div>
